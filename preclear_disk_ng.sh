...
Options:
  ...
  --start-step N            Start at step N (default: 1)
  --resume                  Resume from last state
  --no-badblocks            Skip badblocks test
  --no-smart-long           Skip SMART long test
  --no-fio                  Skip FIO probe
  --no-hdparm               Skip hdparm tuning
  --pause-c N               Temperature pause threshold (default: 50)
  --resume-c N              Temperature resume threshold (default: 45)
  --abort-c N               Temperature abort threshold (default: 55)
  --no-direct               Don't use direct I/O
  --help                    Show this help
EOF
}
parse_args() {
  ...
  for cycle in $(seq 1 "$CYCLES"); do
    ...
    if (( START_STEP <= 1 )); then
      if ! step_preread; then
        local rc=$?
        if [[ $rc -eq 75 ]]; then echo "TEMP abort triggered. Resume later with --resume"; exit 75; fi
